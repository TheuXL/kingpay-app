...
export default function AdminDashboardScreen() {
  ...
  const { 
    ...
    fetchTopSellersReport // Usar o endpoint GET para o admin dashboard
  } = useDashboardStore();

  // Carregar dados do dashboard ao montar o componente
  useEffect(() => {
    fetchAllDashboardData();
    // Usar o endpoint GET específico para o admin dashboard
  ...
  return (
    <ScreenLayout>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.cardsContainer}>
          <DashboardCard
            title="Projeção Mês Atual"
            value={formatCurrency(dashboardData?.projecoes_financeiras?.mes_atual)}
            icon="chart-line"
            color="#AA00FF"
            isLoading={isLoading}
          />
          <DashboardCard
            ...
            value={formatCurrency(dashboardData?.projecoes_financeiras?.proximo_mes)}
        ...
        <DashboardChart
          data={graficoData}
          isLoading={isLoading}
          title="Vendas por Período (Admin)"
        />
